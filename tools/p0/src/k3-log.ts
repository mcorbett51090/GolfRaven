...
function parseIsoDateOrNull(raw: string, context: string): string | null {
  ...
  if (!ISO_DATE_RE.test(cell)) {
    throw new Error(`${context}: malformed date "${cell}" — expected ISO "YYYY-MM-DD" or a blank cell.`);
  }
...
function parseSearchConsoleTable(lines: string[]): { rows: K3SearchConsoleRow[]; readDate: string | null } {
  ...
  while (i < lines.length && lines[i]!.trim().startsWith("|")) {
    ...
    if (cells.length !== 3) {
      throw new Error(`docs/p0/K3.md Search Console row "${line.trim()}" has ${cells.length} cells, expected 3.`);
    }
    const [monthRaw, clicksRaw, readDateRaw] = cells as [string, string, string];
    ...
    const readDate = parseIsoDateOrNull(readDateRaw, `Search Console ${monthRaw}: Read date`);
    if (readDate !== null) readDateEntries.push({ month: monthRaw, date: readDate });
    i += 1;
  }
  assertNoStrayRowsAfterTable(lines, i, 'docs/p0/K3.md "## Search Console read"');
  ...
  const rows = K3_SEARCH_CONSOLE_MONTHS.map((month) => ({ month, clicks: byMonth.get(month)! }));
...
function parseKeywordTable(lines: string[]): K3KeywordRow[] {
  ...
  while (i < lines.length && lines[i]!.trim().startsWith("|")) {
    ...
    const cells = splitRow(line);
    if (cells.length !== 3) {
      throw new Error(`docs/p0/K3.md Keyword Planner row "${line.trim()}" has ${cells.length} cells, expected 3.`);
    }
    ...
    byTerm.set(termNormalized, { term: termNormalized as (typeof K3_KEYWORD_TERMS)[number], lowerBound, upperBound });
    i += 1;
  }
  assertNoStrayRowsAfterTable(lines, i, 'docs/p0/K3.md "## Keyword Planner read"');
