...
const distBuilt =
  existsSync(path.join(DIST, "x5-overpass.js")) &&
  existsSync(path.join(DIST, "x1-ios-export.js"));

describe.skipIf(!distBuilt)(
  "CLI integration (requires `pnpm build` first)",
  () => {
    it("x5-overpass n-osm --from-file prints N_osm with no network call", async () => {
      ...
      expect(stdout).toContain(
        "N_osm (US+CA leisure=golf_course count): 16212",
      );
    });
    ...
    it("x1-ios-export CLI refuses (non-zero exit) against the real, pre-round docs/p0/X1.md with no recorded-export date logged", async () => {
      ...
      await expect(
        ...
        stderr: expect.stringMatching(/Recorded export|uncommitted changes/),
      });
    ...
    it("x1-ios-export CLI --informational runs against the real, pre-round docs/p0/X1.md and marks recorded: false", async () => {
      const outPrefix = path.join(
        OUT_DIR,
        "x1-ios-export-informational-result",
      );
      const { stdout } = await execFileAsync("node", [
        path.join(DIST, "x1-ios-export.js"),
        FIXTURES,
    ...
    it("x1-ios-export CLI refuses --since without --informational", async () => {
      await expect(
        ...
        stderr: expect.stringContaining("--since is refused"),
      });
    });
    ...
    it("x1-ios-export CLI allows --since WITH --informational", async () => {
      const outPrefix = path.join(
        OUT_DIR,
        "x1-ios-export-since-informational-result",
      );
      const { stdout } = await execFileAsync("node", [
        path.join(DIST, "x1-ios-export.js"),
        FIXTURES,
    ...
    // one of them.
    const minimalAndroidJson = path.join(
      OUT_DIR,
      "x1-verdict-minimal-android.json",
    );
    const minimalSourceMapJson = path.join(
      OUT_DIR,
      "x1-verdict-minimal-source-map.json",
    );
    writeFileSync(
      minimalAndroidJson,
      JSON.stringify({
    ...
    it("x1-verdict CLI refuses when --android's JSON has no os field", async () => {
      const badAndroidJson = path.join(
        OUT_DIR,
        "x1-verdict-android-no-os.json",
      );
      writeFileSync(
        badAndroidJson,
        JSON.stringify({
      ...
      await expect(
        ...
        stderr: expect.stringContaining('not "android"'),
      });
    });
    ...
    it("x1-verdict CLI refuses when --android's JSON has no generatedAt field", async () => {
      const badAndroidJson = path.join(
        OUT_DIR,
        "x1-verdict-android-no-generated-at.json",
      );
      writeFileSync(
        badAndroidJson,
        JSON.stringify({
      ...
      await expect(
        ...
        stderr: expect.stringContaining("no generatedAt"),
      });
    });
    ...
    it("x1-verdict CLI refuses --informational on a real (non-fixture) Android path while unbound", async () => {
      await expect(
        ...
        stderr: expect.stringContaining("no informational runs on real data"),
      });
    });
    ...
    it("k1-verdict CLI: empty log, as-of 2026-09-24 → pending, no MISS/PASS", async () => {
      ...
      const { stdout } = await execFileAsync("node", [
        ...
        "--log",
        path.join(FIXTURES, "k1-log-empty.md"),
        "--as-of",
        "2026-09-24",
        "--out",
        outPrefix,
    ...
    it("k1-verdict CLI refuses a --as-of later than today", async () => {
      ...
      await expect(
        ...
        stderr: expect.stringContaining("later than today"),
      });
      expect(existsSync(`${outPrefix}.json`)).toBe(false);
    });
    ...
    it("k3-verdict CLI refuses (non-zero exit) on a memo with a blank property id", async () => {
      ...
      await expect(
        execFileAsync("node", [
          ...
          "--memo",
          path.join(FIXTURES, "k3-memo-blank.md"),
          "--out",
          outPrefix,
        ]),
      ).rejects.toMatchObject({
        stderr: expect.stringContaining("property id"),
      });
      expect(existsSync(`${outPrefix}.json`)).toBe(false);
    });
  },
