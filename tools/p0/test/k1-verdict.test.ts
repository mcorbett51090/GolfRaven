...
describe("computeK1Verdict: early read (decision 0001 Addendum D R1, cutoff 2026-10-19)", () => {
  it("0 acceptances -> early miss (once the window has closed)", () => {
    const result = verdict(baseOperatorRows(), PAST_BOTH_WINDOWS);
...
describe("computeK1Verdict: as-of pending states (decision 0001 Addendum I)", () => {
  ...
  it("as of 2026-11-30, the early read has closed but the full gate is still pending", () => {
    const result = verdict(baseOperatorRows(), "2026-11-30");
  ...
  it("as of 2026-12-01, the full gate has closed", () => {
    const result = verdict(baseOperatorRows(), "2026-12-01");
    expect(result.fullGate.state).not.toBe("pending");
  });

  it("refuses a malformed --as-of", () => {
    expect(() => verdict(baseOperatorRows(), "12/01/2026")).toThrow(/as-of/);
  });
...
describe("computeK1Verdict: pending markdown never says MISS or PASS", () => {
  it("prints 'n so far' for both reads while pending, and contains no MISS/PASS", () => {
    const result = verdict(
      baseOperatorRows({
        "Tennessee Golf Trail": { callAcceptedDate: "2026-10-06" },
      }),
    ...
    expect(md).toContain("1 so far");
    expect(md).not.toMatch(/\bMISS\b/);
    expect(md).not.toMatch(/\bPASS\b/);
  });
});
...
describe("computeK1Verdict: date sanity (decision 0001 Addendum I)", () => {
  it("throws on a date before 2026-09-23", () => {
    ...
    expect(() => verdict(rows, PAST_BOTH_WINDOWS)).toThrow(/before 2026-09-23/);
  });
  ...
  it("throws when a logged date is later than as-of", () => {
    ...
    expect(() => verdict(rows, "2026-10-15")).toThrow(/after the as-of date/);
  });
  ...
  it("throws when a row's logged date is not a real calendar date", () => {
    const rows = baseOperatorRows({
  ...
  it("throws when a sponsor conversation is dated before that sponsor's own contacted date", () => {
    const rows = [
      ...
      row("Alabama Tourism Department", "Sponsor", {
        ...
        sponsorBudgetStated: "Y",
        sponsorAttributionInterest: "Y",
        sponsorConversationDate: "2026-10-05",
      }),
    ];
    expect(() => verdict(rows, PAST_BOTH_WINDOWS)).toThrow(
      /Sponsor conversation date .* is before Contacted date/,
    );
  });
});
