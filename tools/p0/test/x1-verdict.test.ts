...
function iosWorkout(
  overrides: Partial<X1IosWorkoutRecord>,
): X1IosWorkoutRecord {
  return {
...
const ROUND_WINDOWS: RoundWindow[] = [
  { startIso: "2026-09-20T00:00:00Z", endIso: "2026-09-22T00:00:00Z" },
];
...
describe("x1-verdict: round windows are labels, not a filter (decision 0005, superseding Addendum F)", () => {
  ...
  it("a historical iOS workout OUTSIDE every window still counts toward the verdict (decision 0005)", () => {
    ...
    const ios = iosResult([
      iosWorkout({
        sourceName: "Garmin Connect",
        startDate: "2020-01-01 09:00:00 -0400",
      }),
    ]);
    const android = androidResult([]);
    const result = computeX1Verdict({
  ...
  it("newestCountedWorkoutDateBySource reports the newest start date per source per OS", () => {
    ...
    expect(result.newestCountedWorkoutDateBySource.garmin.ios).toBe(
      "2026-09-20 09:00:00 -0400",
    );
    expect(result.newestCountedWorkoutDateBySource.garmin.android).toBe(
      "2021-05-01T09:00:00Z",
    );
    expect(result.newestCountedWorkoutDateBySource.appleWatch).toEqual({
      ios: null,
      android: null,
    });
  });
...
describe("x1-verdict: decision 0001 Addendum D R6 — CONSENT_REQUIRED follow-up", () => {
  it("a CONSENT_REQUIRED session with a follow-up read of >= 1 point counts as route present", () => {
    ...
    const result = computeX1Verdict({
      ...
      sourceMap: BASE_SOURCE_MAP,
      roundWindows: ROUND_WINDOWS,
      androidRouteFollowUps: {
        "consent-1": { routePresent: true, routePointCount: 1 },
      },
    });
    expect(result.perSource.garmin.android).toBe("pass");
  });
  ...
  it("a CONSENT_REQUIRED session with a follow-up read of 0 points does NOT count as route present", () => {
    ...
    const result = computeX1Verdict({
      ...
      roundWindows: ROUND_WINDOWS,
      androidRouteFollowUps: {
        "consent-2": { routePresent: false, routePointCount: 0 },
      },
    });
...
describe("x1-verdict: decision 0001 Addendum D R6 — the phone-app source and the Hole19 swap", () => {
  ...
  it("Hole19 as appUsed WITHOUT the pre-round swap log throws (literal R6 violation)", () => {
    ...
    ).toThrow(/hole19SwapLoggedBeforeRound/);
  });
...
describe("x1-verdict: recordedVerdicts is purely data-driven — stamps are IGNORED entirely (round-2 Opus-gate correction, post-67bdb27, reflects x1probe2.mjs)", () => {
  it("recordedVerdicts and overallVerdict are the SAME regardless of any recorded/os field the inputs carry", () => {
    ...
    const iosData = [
      iosWorkout({
        sourceName: "Garmin Connect",
        startDate: "2019-05-01 10:00:00 +0000",
        routePresent: true,
        verdict: "pass",
      }),
      iosWorkout({
        sourceName: "Garmin Connect",
        startDate: "2026-09-20 10:00:00 +0000",
        routePresent: false,
        verdict: "fail",
      }),
    ];
    ...
    const scenarios: Array<
      [string, X1VerdictInput["ios"], X1VerdictInput["android"]]
    > = [
      ...
      [
        "hand-stamped recorded:true on both (as JSON someone could edit by hand)",
        {
          ...
          recorded: true,
          os: "ios",
        } as X1VerdictInput["ios"],
        {
          ...androidResult(androidData),
          recorded: true,
        } as X1VerdictInput["android"],
      ],
      [
    ...
    for (const r of results) {
      expect(r.recordedVerdicts.ios.verdict).toBe(
        results[0]!.recordedVerdicts.ios.verdict,
      );
      expect(r.recordedVerdicts.android.verdict).toBe(
        results[0]!.recordedVerdicts.android.verdict,
      );
  ...
  it("recordedVerdicts[os].sourcesPassing mirrors sourcesPassingByOs[os]", () => {
    ...
    expect(result.recordedVerdicts.ios.sourcesPassing).toBe(
      result.sourcesPassingByOs.ios,
    );
    expect(result.recordedVerdicts.android.sourcesPassing).toBe(
      result.sourcesPassingByOs.android,
    );
  });
});
...
describe("x1-verdict: newest-date excludes route-less workouts (should-fix, Opus gate post-d0de4b8)", () => {
  it("newestCountedWorkoutDateBySource only considers route-present entries; a NEWER route-less workout does not pull it forward", () => {
    ...
    expect(result.newestRouteLessWorkoutDateBySource.garmin.ios).toBe(
      "2026-09-22 09:00:00 -0400",
    );
  });
  ...
  it("a source with only route-less workouts has a null newestCountedWorkoutDate but a populated newestRouteLessWorkoutDate", () => {
    ...
    expect(result.newestRouteLessWorkoutDateBySource.phoneApp.ios).toBe(
      "2026-09-20 09:00:00 -0400",
    );
  });
});
...
describe("computeOverallFromBoundResults (round-3 Opus-gate correction, post-8e5a29b) — 'the overall result is a pass if any bound OS recomputes to a pass'", () => {
  ...
  it("iOS bound as kill and Android bound as pass -> overall pass, only when both bound files verify (i.e. both are present in boundResults)", () => {
    expect(
      computeOverallFromBoundResults({ ios: "kill", android: "pass" }),
    ).toBe("pass");
  });

  it("kill when both bound OSes recomputed to kill", () => {
    expect(
      computeOverallFromBoundResults({ ios: "kill", android: "kill" }),
    ).toBe("kill");
  });

  it("pass when both bound OSes recomputed to pass", () => {
    expect(
      computeOverallFromBoundResults({ ios: "pass", android: "pass" }),
    ).toBe("pass");
  });
});
...
function tmpMinimalSourceMap(): string {
  const dir = mkdtempSync(
    path.join(tmpdir(), "golfraven-x1-verdict-cli-inputs-"),
  );
...
function tmpMinimalAndroidJson(
  dir: string,
...
describe("runX1VerdictCli (round-4 Opus-gate correction, post-4279773) — 'verdicts come only from a committed binding'", () => {
  it("refuses a recorded run while docs/p0/X1.md has uncommitted changes", async () => {
    ...
    const outPrefix = path.join(
      mkdtempSync(path.join(tmpdir(), "golfraven-x1-verdict-out-")),
      "result",
    );
    ...
    await expect(
      runX1VerdictCli(
        {
          iosExportDir: undefined,
          androidPath: androidJson,
          sourceMapPath,
          outPrefix,
          informational: false,
        },
