...
# ---------------------------------------------------------------------------
"${PSQL[@]}" -c "
  INSERT INTO app.evidence (user_id, device_id, source, source_ref, status, catalog_version, local_date)
  VALUES ('$USER_ID', '$DEVICE_ID', 'foreground_checkin', '${SOURCE_REF}-seq', 'accepted', 1, current_date)
...
INSERT_SQL="INSERT INTO app.evidence (user_id, device_id, source, source_ref, status, catalog_version, local_date)
  VALUES ('$USER_ID', '$DEVICE_ID', 'foreground_checkin', '$CONC_REF', 'accepted', 1, current_date)
  ON CONFLICT (user_id, source, source_ref) DO NOTHING;"
