 ...
 * hop) as of this pin.
 */
const BACKWARD_LINKS = tzdbBackward.links;
export const TZDB_BACKWARD_LINKS_VERSION = tzdbBackward.tzdbVersion;
export function canonicalizeTimeZone(tz) {
    ...
    for (;;) {
        ...
        if (next === undefined || seen.has(current))
            break; // defensive: the real table has no cycles
        seen.add(current);
        current = next;
    }
    return current;
...
export function tzLikelyContainsCoordinates(tz, coord) {
