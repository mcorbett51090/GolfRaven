...
import { bookingFixSatisfiesHardWindow, classifyEvidenceRow, CORROBORATION_ELIGIBLE_THRESHOLD, DEVICE_GPS_SUBTOTAL_CAP, EVIDENCE_ROW_CAP, finish, fixesOfEvidenceRow, GROUP, isQualityCoSignalFix, OVERALL_SCORE_CAP, resolveFixGrade, staffFixSatisfiesHardWindow, WEIGHT, windowMs, } from "./internal/classify.js";
...
function effectiveVoidReason(row) {
...
function compareCandidates(a, b) {
    ...
    if (aHasFix !== bHasFix)
        return aHasFix ? -1 : 1; // a row WITH a fix always sorts first
    if (aHasFix && bHasFix) {
    ...
    if (a.id < b.id)
        return -1; // ascending id, final tiebreak
    if (a.id > b.id)
        return 1;
    return 0;
}
function voidDuplicateFingerprints(evidence) {
    ...
    const allVoidFingerprints = new Set();
    const winnerIdByFingerprint = new Map();
    for (const [fingerprint, rows] of byFingerprint) {
        if (rows.length < 2)
            continue; // not actually a duplicate group
        const poisons = (r) => r.status === "void" && effectiveVoidReason(r) !== "duplicate";
        ...
        if (candidates.length === 0)
            continue; // every copy was a dedup-void; nothing to pick, nothing to poison
    ...
    return evidence.map((row) => {
        if (row.source !== "receipt_green_fee" || !row.fingerprint)
            return row;
        if (allVoidFingerprints.has(row.fingerprint))
            return { ...row, status: "void" };
        const winnerId = winnerIdByFingerprint.get(row.fingerprint);
        if (winnerId === undefined || row.id === winnerId)
            return row;
        return { ...row, status: "void" };
    });
}
 ...
 * 0.30) = 0.80`. */
function correlationFixIdsOfRow(row) {
    switch (row.source) {
        case "staff_presence":
 ...
 */
function deriveGroups(evidence, contributions, ctx) {
    ...
    for (const idxs of byFixId.values())
        for (let k = 1; k < idxs.length; k += 1)
            dsu.union(idxs[0], idxs[k]);
    ...
    for (const idxs of byPayment.values())
        for (let k = 1; k < idxs.length; k += 1)
            dsu.union(idxs[0], idxs[k]);
    const roundRows = [];
    evidence.forEach((row, i) => {
    ...
    for (let a = 0; a < roundRows.length; a += 1) {
        for (let b = a + 1; b < roundRows.length; b += 1) {
            if (roundRows[a].facilityId === roundRows[b].facilityId &&
        ...
        if (row.source === "staff_presence" && row.coSignalFix === undefined) {
            for (let j = 0; j < n; j += 1) {
                ...
                for (const fix of fixesOfRow(evidence[j])) {
                    if (staffFixSatisfiesHardWindow(fix, row.scanAt, ctx))
                        dsu.union(i, j);
                }
            }
        }
        if (row.source === "booking" && row.presenceFix === undefined && row.localDate === ctx.playLocalDate) {
            for (let j = 0; j < n; j += 1) {
                if (j === i)
                    continue;
                for (const fix of fixesOfRow(evidence[j])) {
                    if (bookingFixSatisfiesHardWindow(fix, ctx))
                        dsu.union(i, j);
                }
            }
        }
...
function pickBestSatisfyingFix(fixes, satisfies) {
...
function resolveGroups(evidence, contributions, groups, ctx) {
    ...
    for (const idxs of byGroup.values()) {
        ...
        for (const i of idxs) {
            ...
            if (row.source === "staff_presence") {
                const satisfyingFix = pickBestSatisfyingFix(idxs.flatMap((j) => fixesOfRow(evidence[j])), (fix) => staffFixSatisfiesHardWindow(fix, row.scanAt, ctx));
                if (satisfyingFix)
                    candidates.push({ row, contribution, classId: "staff_presence_hard", satisfyingFix });
            }
            else if (row.source === "booking" && row.localDate === ctx.playLocalDate) {
                const satisfyingFix = pickBestSatisfyingFix(idxs.flatMap((j) => fixesOfRow(evidence[j])), (fix) => bookingFixSatisfiesHardWindow(fix, ctx));
                if (satisfyingFix)
                    candidates.push({ row, contribution, classId: "booking_hard", satisfyingFix });
            }
        }
        ...
        if (candidates.length > 0) {
            ...
            // deterministic).
            const effectiveWeight = (c) => (c.row.courseDisambiguatedBy === "user" ? -1 : WEIGHT[c.classId]);
            let winner = candidates[0];
            for (const c of candidates)
                if (effectiveWeight(c) > effectiveWeight(winner))
                    winner = c;
            resolved.push({
        ...
        for (const i of idxs)
            resolved.push({ contribution: contributions[i], groupId: outputGroupId });
    }
    return resolved;
}
/* ------------------------------------------------------------------ */
...
function pickMaxPerGroup(contributions, groups, weightOf) {
    const byGroup = new Map();
    contributions.forEach((c, i) => {
        const g = groups[i];
        const arr = byGroup.get(g) ?? [];
...
function combine(contributions, groups, pipeline) {
    const weightOf = (c) => (pipeline === "badge" ? c.badgeWeight : c.moneyWeight);
...
/* ------------------------------------------------------------------ */
function collectFixes(evidence) {
    const fixes = [];
    for (const row of evidence)
        fixes.push(...fixesOfRow(row));
    return fixes;
}
 ...
 * proof. */
function qualifyingPresenceFixes(evidence, ctx) {
    return collectFixes(evidence).filter((fix) => isQualityCoSignalFix(fix, ctx.playFacilityId) && fix.localDate === ctx.playLocalDate);
}
/**
 ...
 */
function computeHeldReview(playContributions, moneyMerged, presenceFixes, hardSignal, money) {
    if (!money)
        return false;
...
function corroborationApplies(ctx) {
 ...
 * | `fixId` | Yes | the challenge id or assertion hash the attestation exchange itself produced — never a client-invented string (M1). **Ninth gate, item 3: PINNED to unpadded base64url (RFC 4648 §5) — `[A-Za-z0-9_-]`, no `+`/`/`/`=`.** This is a HARD REQUIREMENT on the Edge Function/attestation layer that produces this value — standard, padded base64 (any `+`/`/`/`=`) is rejected and the row quarantined (`parse-evidence.ts`'s `FixIdSchema`); see `docs/security/p3-money-path-requirements.md` §1 for the rationale (why base64url, not hex, was chosen) |
 ...
 * | `ctx.playCourseId` | Yes (H3 residual, seventh gate: now REQUIRED in BOTH the parser schema and the TypeScript type, item 8) | the `play` row's resolved course, at a multi-course (e.g. 36-hole) facility — a `courses` table lookup, never client-supplied; a row WITHOUT its own `courseId` is facility-level evidence and stays allowed regardless (H3's residual rule) — the DB's `play_evidence UNIQUE(evidence_id)` constraint (migration 0017) is the separate guarantee that stops the SAME evidence row being attributed to two different plays at all. **Ninth gate, item 2: a SQL NULL `course_id` column MUST be serialized as an OMITTED `courseId` JSON key, never as a literal `null`.** This package treats the two completely differently on purpose: an OMITTED `courseId` means "no course anchor, facility-level evidence" (H3's residual rule, always a candidate); a PRESENT `courseId: null` is a malformed value (not a string) and is quarantined like any other shape violation. A DB/ORM layer that naively serializes `NULL` as JSON `null` instead of dropping the key turns every facility-level row into a quarantine candidate for no reason — see `docs/security/p3-money-path-requirements.md` §2 |
 ...
 * `fraud_signal` is not.
 */
export function scorePlay(evidenceIn, ctx) {
...
function computeInputDigest(evidence, ctx) {
 ...
 */
function scorePlayOnParsedInput(evidenceIn, ctx) {
    ...
    const corroborationEligible = hasPlayClass && playClassesBadge >= CORROBORATION_ELIGIBLE_THRESHOLD && corroborationApplies(ctx);
    const allContributions = [...playContributions];
