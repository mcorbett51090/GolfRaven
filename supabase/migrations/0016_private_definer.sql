...
CREATE POLICY pd_setnull_attestation_staff_user_id_r ON app.attestation FOR SELECT TO private_definer USING (staff_user_id = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_entitlement_redeemed_by_staff_r ON app.entitlement FOR SELECT TO private_definer USING (redeemed_by_staff = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_fraud_signal_cleared_by_r ON app.fraud_signal FOR SELECT TO private_definer USING (cleared_by = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_marker_code_activated_by_staff_r ON app.marker_code FOR SELECT TO private_definer USING (activated_by_staff = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_marker_code_redeemed_by_r ON app.marker_code FOR SELECT TO private_definer USING (redeemed_by = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_offer_code_redeemed_by_staff_r ON app.offer_code FOR SELECT TO private_definer USING (redeemed_by_staff = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_partner_member_invited_by_r ON app.partner_member FOR SELECT TO private_definer USING (invited_by = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_review_item_resolved_by_r ON app.review_item FOR SELECT TO private_definer USING (resolved_by = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_setnull_special_marker_stock_movement_by_member_r ON app.special_marker_stock_movement FOR SELECT TO private_definer USING (by_member = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_fraud_signal_update_r ON app.fraud_signal FOR SELECT TO private_definer USING (user_id = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_attestation_player_update_r ON app.attestation FOR SELECT TO private_definer USING (player_user_id = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_audit_log_update_r ON app.audit_log FOR SELECT TO private_definer USING (actor_user_id = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
CREATE POLICY pd_shift_log_update_r ON app.attestation_shift_log FOR SELECT TO private_definer USING (player_pseudonym = current_setting('app.delete_my_data.target_pseudonym', true) OR (player_pseudonym IS NULL AND player_handle_snapshot = current_setting('app.delete_my_data.target_handle', true)));
...
CREATE POLICY pd_receipt_fingerprint_update_r ON app.receipt_fingerprint FOR SELECT TO private_definer USING (user_id = current_setting('app.delete_my_data.target_user_id', true)::uuid);
...
INSERT INTO private.definer_policy_allowlist (schema_name, table_name, policy_name, command, scoped, note) VALUES
  ...
  ('app', 'attestation', 'pd_setnull_attestation_staff_user_id_r', 'SELECT', true, 'row-visibility companion to pd_setnull_attestation_staff_user_id (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'entitlement', 'pd_setnull_entitlement_redeemed_by_staff_r', 'SELECT', true, 'row-visibility companion to pd_setnull_entitlement_redeemed_by_staff (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'fraud_signal', 'pd_setnull_fraud_signal_cleared_by_r', 'SELECT', true, 'row-visibility companion to pd_setnull_fraud_signal_cleared_by (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'marker_code', 'pd_setnull_marker_code_activated_by_staff_r', 'SELECT', true, 'row-visibility companion to pd_setnull_marker_code_activated_by_staff (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'marker_code', 'pd_setnull_marker_code_redeemed_by_r', 'SELECT', true, 'row-visibility companion to pd_setnull_marker_code_redeemed_by (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'offer_code', 'pd_setnull_offer_code_redeemed_by_staff_r', 'SELECT', true, 'row-visibility companion to pd_setnull_offer_code_redeemed_by_staff (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'partner_member', 'pd_setnull_partner_member_invited_by_r', 'SELECT', true, 'row-visibility companion to pd_setnull_partner_member_invited_by (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'review_item', 'pd_setnull_review_item_resolved_by_r', 'SELECT', true, 'row-visibility companion to pd_setnull_review_item_resolved_by (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'special_marker_stock_movement', 'pd_setnull_special_marker_stock_movement_by_member_r', 'SELECT', true, 'row-visibility companion to pd_setnull_special_marker_stock_movement_by_member (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'fraud_signal', 'pd_fraud_signal_update_r', 'SELECT', true, 'row-visibility companion to pd_fraud_signal_update (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'attestation', 'pd_attestation_player_update_r', 'SELECT', true, 'row-visibility companion to pd_attestation_player_update (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'audit_log', 'pd_audit_log_update_r', 'SELECT', true, 'row-visibility companion to pd_audit_log_update (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'attestation_shift_log', 'pd_shift_log_update_r', 'SELECT', true, 'row-visibility companion to pd_shift_log_update (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
  ...
  ('app', 'receipt_fingerprint', 'pd_receipt_fingerprint_update_r', 'SELECT', true, 'row-visibility companion to pd_receipt_fingerprint_update (UPDATE ... WHERE needs SELECT-level visibility, confirmed empirically)'),
