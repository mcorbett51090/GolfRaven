...
describe("B3: analytics-schema.json and RULES pin the EXACT same key set per event", () => {
  ...
  it("PROOF: an event present in the schema but missing from RULES is caught", () => {
    ...
    const mutatedRuntimeEvents = Object.keys(RULES).filter(
      (e) => e !== "csp_violation",
    );
    const missing = schemaEvents.filter(
      (e) => !mutatedRuntimeEvents.includes(e),
    );
  ...
  it("PROOF: an EXTRA key on an event (not in analytics-schema.json) is caught — the exact drift this gate exists for", () => {
    const schema = JSON.parse(readFileSync(SCHEMA_PATH, "utf8")) as Record<
      ...
      unknown
    >;
    const mutatedBookingClickKeys = [
      ...Object.keys(RULES.booking_click!),
      "lat",
      "user_id",
    ].sort();
    const schemaBookingClickKeys = [
      ...(schema.booking_click as string[]),
    ].sort();
    const extraInRuntime = mutatedBookingClickKeys.filter(
      (k) => !schemaBookingClickKeys.includes(k),
    );
  ...
  it("every RULES event name and key also appears, verbatim, in analytics-schema.json (drift proof the other direction)", () => {
    ...
    for (const [event, spec] of Object.entries(RULES)) {
      expect(
        schema[event],
        `event "${event}" missing from analytics-schema.json`,
      ).toBeDefined();
...
describe("B3: looksLikePii() rejects coordinate-, email- and opaque-id-shaped values", () => {
  it.each([
    ...
    [
      "a 32-char hex id (e.g. a facility/course id)",
      "5f2b1c9a8e7d6f4c3b2a19087f6e5d4c",
    ],
  ...
  it.each([
    ...
    "%s is NOT flagged (no false positives on ordinary enum/slug values)",
    (_label, value) => {
  ...
  it("project() drops the WHOLE event when a pattern-typed field is fed a PII-shaped value", () => {
    ...
    const projectWith = (
      rules: Record<string, EventRules>,
      name: string,
      ...
      for (const [key, rule] of Object.entries(spec)) {
        if (
          ...
          !rule.value.test(raw[key] as string)
        ) {
          continue;
        }
    ...
    expect(
      projectWith(permissive, "probe_event", { free_field: "36.104729" }),
    ).toBeNull();
    expect(
      projectWith(permissive, "probe_event", {
        free_field: "jane@example.com",
      }),
    ).toBeNull();
    expect(
      projectWith(permissive, "probe_event", { free_field: "not-pii-at-all" }),
    ).toEqual({
...
describe('B3 PROOF: "adding lat or user_id to booking_click" is caught by the PII shape guard', () => {
  it("a booking_click rule set widened to admit lat/user_id still drops the whole event when fed real PII-shaped values", () => {
    ...
    const localRules: Record<string, EventRules> = {
      booking_click: bypassedRules,
    };
...
describe("AT(8): createAnalytics()'s dataLayer projection never leaks PII", () => {
  ...
  it("generate_lead: exactly the schema keys carry values, and NO PII field survives", () => {
    ...
    a.track("generate_lead", {
      ...
      message:
        "I own this and the tee times are wrong, call me on 706-867-9862",
    });
    ...
    expect(blob).not.toMatch(
      /Jane Doe|jane@example\.com|tee times are wrong|706-867-9862|Ridge Overlook/,
    );
