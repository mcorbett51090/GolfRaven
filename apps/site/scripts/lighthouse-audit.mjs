...
async function main() {
  if (!existsSync(distDir)) {
    console.error(`lighthouse-audit: ${distDir} does not exist — run \`pnpm build\` first.`);
    process.exit(1);
  }
  ...
  console.log(`lighthouse-audit: serving ${distDir} on http://127.0.0.1:${port}`);
  ...
  const pages = [
    { path: "/", label: "hub" },
    { path: process.env.LH_TRAIL_PATH ?? "/trails/fictional-ridge-golf-trail/", label: "trail" },
    { path: process.env.LH_COURSE_PATH ?? "/courses/ridge-overlook-golf-club/", label: "course" },
  ];

  const results = [];
  for (const p of pages) {
    if (!existsSync(join(distDir, p.path.slice(1), "index.html")) && p.path !== "/") {
      console.log(`lighthouse-audit: skipping ${p.label} (${p.path}) — not present in this dist`);
      continue;
    }
  ...
  for (const r of results) {
    if (r.error) {
      console.log(`  ${r.label.padEnd(6)} ${r.path.padEnd(40)} ERROR: ${r.error}`);
    } else {
