...
/**
 * Reads and validates `GOLFRAVEN_TILE_STYLE_URL`. An unset/empty value is
 * the supported "no tile provider yet" state (`configured: false`) — never
 * an error. A SET value that isn't a valid `https:` URL IS an error (a
 * config typo silently degrading to "no map" would be worse than failing
 * the build loudly).
 *
...
export function tileConfig(env = process.env) {
  ...
  if (!raw) {
    return { configured: false, attribution };
  }
  /** @type {URL} */
  let parsed;
  try {
  ...
  return { configured: true, styleUrl: raw, host: parsed.host, attribution };
}
