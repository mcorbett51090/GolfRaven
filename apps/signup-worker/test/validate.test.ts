...
describe("validateSignupPayload", () => {
  ...
  it("rejects a missing/invalid email", () => {
    expect(validateSignupPayload({ ...VALID, email: undefined }).ok).toBe(
      false,
    );
    expect(validateSignupPayload({ ...VALID, email: "not-an-email" }).ok).toBe(
      false,
    );
  ...
  it("rejects ageConfirmed that is not the literal boolean true", () => {
    expect(validateSignupPayload({ ...VALID, ageConfirmed: "true" }).ok).toBe(
      false,
    );
    expect(validateSignupPayload({ ...VALID, ageConfirmed: 1 }).ok).toBe(false);
    expect(validateSignupPayload({ ...VALID, ageConfirmed: false }).ok).toBe(
      false,
    );
  });
  ...
  it("rejects a consentVersion outside the allow-list", () => {
    expect(
      validateSignupPayload({ ...VALID, consentVersion: "2020-01-01" }).ok,
    ).toBe(false);
    expect(validateSignupPayload({ ...VALID, consentVersion: 123 }).ok).toBe(
      false,
    );
  });

  it("rejects a missing or oversized turnstileToken", () => {
    expect(validateSignupPayload({ ...VALID, turnstileToken: "" }).ok).toBe(
      false,
    );
    expect(
      validateSignupPayload({ ...VALID, turnstileToken: undefined }).ok,
    ).toBe(false);
    expect(
      validateSignupPayload({ ...VALID, turnstileToken: "a".repeat(5000) }).ok,
    ).toBe(false);
  });
  ...
  it("rejects an oversized source", () => {
    expect(
      validateSignupPayload({ ...VALID, source: "x".repeat(100) }).ok,
    ).toBe(false);
  });
});
