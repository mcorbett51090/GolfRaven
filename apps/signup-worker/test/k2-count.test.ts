...
function rowsOf(
  ...
  confirmedAt: string,
  emailPrefix = "user",
): { email_lc: string; confirmed_at: string }[] {
  return Array.from({ length: n }, (_, i) => ({
    email_lc: `${emailPrefix}${i}@example.com`,
...
describe("computeK2Counts", () => {
  it("refuses to run when day 0 is missing", () => {
    expect(() =>
      computeK2Counts({ rows: [], day0: "", excludedAddresses: [] }),
    ).toThrow(/day 0/i);
  });

  it("refuses to run when day 0 is not a valid date", () => {
    expect(() =>
      computeK2Counts({ rows: [], day0: "not-a-date", excludedAddresses: [] }),
    ).toThrow(/valid/i);
  });

  it("never counts unconfirmed rows", () => {
    const rows = [
      { email_lc: "a@example.com", confirmed_at: null },
      { email_lc: "b@example.com" },
    ];
  ...
  it("excludes an address in the excluded list, case-insensitively", () => {
    const rows = [
      {
        email_lc: "owner@example.com",
        confirmed_at: "2026-10-06T00:00:00.000Z",
      },
      {
        email_lc: "real@example.com",
        confirmed_at: "2026-10-06T00:00:00.000Z",
      },
    ];
    const result = computeK2Counts({
      rows,
      day0: DAY0,
      excludedAddresses: ["Owner@Example.com"],
    });
  ...
  it("a confirmation exactly AT the advisory/gate cutoff is excluded (strictly-before rule)", () => {
    const advisoryCutoff = new Date(
      new Date(DAY0).getTime() + 14 * 86400000,
    ).toISOString();
    const gateCutoff = new Date(
      new Date(DAY0).getTime() + 42 * 86400000,
    ).toISOString();
    const rows = [
      { email_lc: "exact-advisory@example.com", confirmed_at: advisoryCutoff },
      { email_lc: "exact-gate@example.com", confirmed_at: gateCutoff },
      {
        email_lc: "one-ms-before-gate@example.com",
        confirmed_at: new Date(
          new Date(gateCutoff).getTime() - 1,
        ).toISOString(),
      },
    ];
    const result = computeK2Counts({ rows, day0: DAY0, excludedAddresses: [] });
  ...
  it("computes advisory/gate pass-bar verdicts against the pre-registered thresholds", () => {
    const belowThreshold = rowsOf(
      ADVISORY_THRESHOLD - 1,
      "2026-10-06T00:00:00.000Z",
    );
    const resultFail = computeK2Counts({
      rows: belowThreshold,
      day0: DAY0,
      excludedAddresses: [],
    });
    ...
    const resultPass = computeK2Counts({
      rows: atGate,
      day0: DAY0,
      excludedAddresses: [],
    });
  ...
  it("N12: treats a confirmed_at with no explicit UTC offset (ambiguous local time) as malformed rather than silently local-parsing it", () => {
    const rows = [
      {
        email_lc: "good@example.com",
        confirmed_at: "2026-10-06T00:00:00.000Z",
      },
      {
        email_lc: "no-offset@example.com",
        confirmed_at: "2026-10-06T00:00:00",
      },
    ];
    const result = computeK2Counts({ rows, day0: DAY0, excludedAddresses: [] });
