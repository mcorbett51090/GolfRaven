...
- **Uses:** the plan's own staging rule, FM-08 (plan §10 conventions): *"Where only part of a phase depends on
  a gate, the phase is split into numbered stages, each with its own gate."* P1.0 is such a stage. The P1
...
- **Partner programme and money-mode grammar (K1):**
  ...
  These are also the only `packages/rules` items near the money path, so P1.0 contains no money-path code.
...
- P1.0 pauses at its next merge if **any** P0 memo:
...
- The S0 guard enforces the three hard limits mechanically: no `CONTRACT_VERSION` above 0, no `data/` or
  `catalog/` content, and no committed key or keyset. They do not rely on review.
