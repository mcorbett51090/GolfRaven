...
2. **Golf keyword read (Matt, 30 min):** open a Google Ads account with **no active spend** (O23), use Google
   Keyword Planner to pull monthly search volume for "golf trail" plus each pilot-candidate trail name from the
   research files (Tennessee Golf Trail, Vancouver Island Golf Trail, Robert Trent Jones Golf Trail, plus the
   reserve Oklahoma Golf Trail and any others worth checking). Keyword Planner without spend may return only a
   volume **range**, not a point estimate — the check uses the **range's lower bound** for every term, then
   sums across terms. Compare the combined total to ≥ 5,000/month.
