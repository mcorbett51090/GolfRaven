...
Columns fixed 2026-09-24, before any read — `tools/p0`'s `k3-verdict` reads this table directly
(`src/k3-log.ts`). The six terms are the closed list (decision 0001, Addendum B); fill either Lower
bound + Upper bound, or Point value alone, never both. Do not add, remove or rename rows.

| Term | Lower bound | Upper bound | Point value |
|---|---|---|---|
| golf trail | | | |
| golf trails | | | |
