...
1. **SWC Search Console read (Matt, 15 min):** export SWC's Google Search Console performance data for the
   three most recent complete calendar months, and take the **median of the three monthly organic-click
   totals** (method pinned in decision 0001, Addendum A, before any data is read). Compare to M = 1,000/month.
2. **Golf keyword read (Matt, 30 min):** open a Google Ads account with **no active spend** (O23), use Google
   Keyword Planner's **"Get search volume and forecasts"** (not "Discover new keywords") to pull monthly search
   volume for the **closed, six-term list fixed in decision 0001, Addendum B** — `golf trail`, `golf trails`,
   `robert trent jones golf trail`, `tennessee golf trail`, `vancouver island golf trail`, `oklahoma golf
   trail` — English only, no additions after data is read. Keyword Planner without spend may return only a
   volume **range**, not a point estimate — the check uses the **range's lower bound** for each of the six
   terms, then sums across all six. Compare the combined total to ≥ 5,000/month.
